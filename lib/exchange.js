...
function exchange(name, type, options) {
  ...
  function createRpcQueue(key, options = {}, completionCallback) {
    return createQueue({
      ...{
        key: key,
        name: key,
  ...
  function rpcServer(key, handler, completionCallback, options = {}) {
    const rpcQueue = createRpcQueue(key, options, completionCallback)
  ...
  function createQueue(options, completionCallback) {
